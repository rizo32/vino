...
function UserForm({ user, onChange, onSubmit, onReturn, message, setMessage }) {
  return (
    <form className="flex flex-col w-10/12 ml-auto mr-auto" onSubmit={onSubmit}>
      ...
      <label htmlFor="email" className="mt-vh-2 ml-2">
        Courriel
        {message.email && (
          <span className="text-red-900 text-sm pl-2">{message.email[0]}</span>
        )}
      </label>
      <input
        type="email"
      ...
      <div className="text-center absolute bottom-14 left-1/2 transform -translate-x-1/2 w-10/12">
        <p className="cursor-pointer" onClick={onReturn}>{"<"} Retour</p>
      </div>
    </form>
  );
}
