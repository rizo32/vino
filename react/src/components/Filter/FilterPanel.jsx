    ...
    (e, filterCategory) => {
      ...
      setFilters((prevFilters) => {
        ...
        if (isChecked) {
          ...
          newFilters[filterCategory] = [...prevFilters[filterCategory], value];
        } else {
      ...
      >
        {categories.map((category) => (
          ...
          >
            ...
            <FontAwesomeIcon icon={getCategoryIcon(category.internalName)} />
          </button>
        ))}
        <button
          className={`${
            anyCategoryIsActive() ? "" : "hidden"
          } p-2 rounded-3xl flex justify-center items-center gap-3 flex-shrink-0 border border-black hover:text-white active:text-white hover:bg-red-900 active:bg-red-900`}
